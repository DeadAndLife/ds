...
int main(void)
{
     ...
     tmp->chinese = i;
     tmp->math = i * i;
     __builtin___snprintf_chk (tmp->name, 32, 0, __builtin_object_size (tmp->name, 2 > 1 ? 1 : 0), "stu%d", i);
     ({ ({ ((&head)->next)->prev = (&tmp->node); (&tmp->node)->next = ((&head)->next); (&tmp->node)->prev = ((&head)); ((&head))->next = (&tmp->node); }); });
      }

      for ((p) = (&head)->next; (p) != (&head); (p) = (p)->next) {
     tmp = ({ const typeof( ((struct score *)0)->node ) *__mptr = (p); (struct score *)( (char *)__mptr - ((size_t) &((struct score *)0)->node) );});
